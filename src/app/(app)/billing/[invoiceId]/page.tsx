...
// Purpose: Invoice detail — line items, GST breakdown, payment recording, UPI QR code, PDF download
...
export default function InvoicePage({ params }: { params: { invoiceId: string } }) {
  ...
  // Regenerate QR when due changes
  useEffect(() => {
    if (!inv) return
  ...
  const pdfProps = pdfData
    ? { ...pdfData, upiQrDataUrl: qrDataUrl }
    : null
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <div className="flex items-start justify-between rounded-lg border border-border bg-background p-5">
        <div>
          <p className="font-mono text-xs text-muted-foreground">{pdfData?.invoiceNumber ?? inv.id.slice(0, 8).toUpperCase()}</p>
          <h2 className="text-lg font-semibold text-foreground">{inv.patients?.name ?? '—'}</h2>
          <p className="text-sm text-muted-foreground">{formatDate(inv.issued_at)} · {inv.patients?.phone}</p>
        </div>
      ...
      <div className="rounded-lg border border-border bg-background">
        ...
        <div className="flex flex-col items-end gap-1 border-t border-border px-5 py-4">
          ...
          <div className="flex w-48 justify-between border-t border-border pt-2 text-base font-semibold text-foreground"><span>Due</span><span>{formatCurrency(due)}</span></div>
        </div>
      </div>
